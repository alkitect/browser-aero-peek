...
- **Firefox WebExt persistence residual:** This tip documents **Temporary Add-on** only (unloads when Firefox quits — reload `.xpi` after restart). Durable install requires a **Mozilla-signed** `.xpi` (AMO listed or unlisted); same gecko id. Do **not** ship `xpinstall.signatures.required=false` / unsigned profile sideload as the product path. Follow-up: plan `FIREFOX-DURABLE` / umbrella `TODO-FIREFOX-DURABLE`.
