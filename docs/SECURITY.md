...
- **Shared MV3 blast radius:** one pinned extension id / `allowed_origins` origin is reused across Chromium-family NM dirs when multiple browsers are enabled (Brave / Chrome / Opera / Opera GX / Vivaldi / Chromium Snap+deb+Flatpak / Edge packaging variants). Compromise of that origin affects every enabled Chromium NM lane. Do not widen origins; do not fork keys casually. Staged copies (`mv3-<id>`) only force the hello `browserId`; they keep the same id/key (except Firefox mozilla lane).
- **NM path exclusivity:** install refuses to write if two enabled lanes resolve to the same NativeMessagingHosts directory (e.g. Chromium Snap must not alias `~/.config/chromium` while `chromium-deb` is enabled).
- **Flatpak spawn boundary:** Flatpak cannot see the host Unix socket (`$XDG_RUNTIME_DIR/...sock`) and cannot import host PyGObject. NM JSON for Flatpak rows points at `~/.local/bin/browser-tabs-nm-flatpak` → `flatpak-spawn --host` → `browser-tabs-nm`. Install applies `filesystem=~/.local/bin:ro`, `filesystem=~/.local/share/alkitect-browser-tabs:ro`, and `--talk-name=org.freedesktop.Flatpak` when the Flatpak app is present (including `com.opera.opera-gx`). Residual trust: same-UID Flatpak app with those overrides can spawn the host NM bridge.
